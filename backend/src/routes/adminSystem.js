...
app.get('/system/security', admin, async c => { const failed=await c.env.DB.prepare('SELECT COUNT(*) n FROM users WHERE failedLoginAttempts>0').first(); const locked=await all(c.env.DB.prepare('SELECT id,name,email FROM users WHERE lockedUntil IS NOT NULL')); const staff=await all(c.env.DB.prepare("SELECT id,name,email,role,staffRole FROM users WHERE role IN ('admin','moderator')")); return ok(c,{ passwordHashing:'PBKDF2-SHA256-210000 (Workers-WebCrypto)', jwt:'HS256 WebCrypto', csrf:'double-submit cookie', cors:'environment allowlist', uploads:'R2 Worker route', bcryptReplaced:true, bcryptNote:'bcryptjs replaced for Workers CPU.', warnings:[], failedLoginsCount:failed?.n||0, lockedAccounts:locked, suspiciousIps:[], adminAccounts:staff, recentLogins:[] }); });
